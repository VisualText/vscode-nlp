...
- **The configuration is called "NLP++: debug (live)".** It was "debug rules (live)", which stopped being accurate when breakpoints reached `@CODE`, `@POST` and `@DECL` -- it debugs an analyzer, not only its rules.
