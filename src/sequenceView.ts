...
export class PassTree implements vscode.TreeDataProvider<SequenceItem> {
	...
	getPasses(passes: PassItem[]): SequenceItem[] {
		...
		const hasPat: boolean = dirfuncs.getFiles(visualText.analyzer.getSpecDirectory(), ['.pat']).length ? true : false;
	...
	insertLibraryFile(seqItem: SequenceItem, dir: string, filename: string) {
		const filepath = path.join(visualText.getVisualTextDirectory(visualText.ANALYZER_SEQUENCE_FOLDER), dir, filename);
		const newfile: vscode.Uri = vscode.Uri.file(filepath);
		const seqFile = visualText.analyzer.seqFile;
		const passNum = seqFile.findPassByFilename(filename);
		// If the pass exists, replace it
		if (passNum) {
			const currentFile = vscode.Uri.file(path.join(visualText.analyzer.getSpecDirectory().fsPath, filename));
		...
		if (visualText.hasWorkspaceFolder()) {
			...
			vscode.window.showOpenDialog(options).then(selection => {
				...
				for (const select of selection.reverse()) {
					const newfile: vscode.Uri = vscode.Uri.file(select.fsPath);
					seqFile.insertPass(seqItem.row, newfile);
				}
				vscode.commands.executeCommand('sequenceView.refreshAll');
			});
	...
	insertSisterPass(seqItem: SequenceItem): void {
		if (visualText.getWorkspaceFolder()) {
			const items: vscode.QuickPickItem[] = visualText.analyzerFolderList(true);
			sequenceView.insertChosenPasses(seqItem, items);
		}
	}
...
export class SequenceView {
	...
	constructor(ctx: vscode.ExtensionContext) {
		...
		ctx.subscriptions.push(this.sequenceView);
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.openPass', (seqItem) => this.openPass(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.openTree', (seqItem) => this.openTree(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.displayMatchedRules', (seqItem) => this.displayMatchedRules(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.openKB', (seqItem) => this.openKB(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.search', () => this.search()));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.searchTop', () => this.searchTop()));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.finalTree', () => this.finalTree()));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.convert', () => this.convertPatToNLP()));

		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.moveUp', (seqItem) => treeDataProvider.moveUp(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.moveDown', (seqItem) => treeDataProvider.moveDown(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.refreshAll', () => treeDataProvider.refresh()));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.insert', (seqItem) => treeDataProvider.insertPass(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.insertSister', (seqItem) => treeDataProvider.insertSisterPass(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.insertNew', (seqItem) => treeDataProvider.insertRules(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.insertCode', (seqItem) => treeDataProvider.insertCode(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.insertDecl', (seqItem) => treeDataProvider.insertDecl(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.insertPython', (seqItem) => treeDataProvider.insertPython(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.insertPythonBeforeTokenize', (seqItem) => treeDataProvider.insertPython(seqItem, true)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.insertPythonLibrary', (seqItem) => treeDataProvider.insertPythonLibrary(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.insertPythonLibraryBeforeTokenize', (seqItem) => treeDataProvider.insertPythonLibrary(seqItem, true)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.insertLibrary', (seqItem) => treeDataProvider.insertLibraryPass(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.libraryKBFuncs', (seqItem) => treeDataProvider.libraryKBFuncs(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.libraryTreeFuncs', (seqItem) => treeDataProvider.libraryTreeFuncs(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.libraryUtilFuncs', (seqItem) => treeDataProvider.libraryUtilFuncs(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.libraryLines', (seqItem) => treeDataProvider.libraryLines(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.libraryLinesDictTokZ', (seqItem) => treeDataProvider.libraryLinesDictTokZ(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.libraryWhiteSpaces', (seqItem) => treeDataProvider.libraryWhiteSpaces(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.delete', (seqItem) => treeDataProvider.deletePass(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.deleteFolder', (seqItem) => treeDataProvider.deleteFolder(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.duplicate', (seqItem) => treeDataProvider.duplicatePass(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.rename', (seqItem) => treeDataProvider.renamePass(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.renameFolder', (seqItem) => treeDataProvider.renameFolder(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.typePat', (seqItem) => treeDataProvider.typePat(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.typeRec', (seqItem) => treeDataProvider.typeRec(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.typeOff', (seqItem) => treeDataProvider.typeOff(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.typeOn', (seqItem) => treeDataProvider.typeOn(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.newFolder', (seqItem) => treeDataProvider.newFolder(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.tokenize', (seqItem) => treeDataProvider.tokenize(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.dicttok', (seqItem) => treeDataProvider.dicttok(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.dicttokz', (seqItem) => treeDataProvider.dicttokz(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.chartok', (seqItem) => treeDataProvider.chartok(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.cmltok', (seqItem) => treeDataProvider.cmltok(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.explore', () => this.explore()));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.insertOrphan', (seqItem) => this.insertOrphan(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.toggleActive', (seqItem) => this.toggleActive(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.modAdd', () => this.modAdd()));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.video', () => this.video()));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.compareSisters', (seqItem) => this.compareSisters(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.copyContext', (seqItem) => this.copyContext(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.compareLibrary', (seqItem) => this.compareLibrary(seqItem)));
		ctx.subscriptions.push(vscode.commands.registerCommand('sequenceView.updateTitle', () => this.updateTitle()));
	}
	...
	compareSisters(seqItem: SequenceItem) {
		if (visualText.getWorkspaceFolder()) {
			const items: vscode.QuickPickItem[] = visualText.analyzer.seqFile.getSisterFiles(seqItem.uri.fsPath);
	...
	insertAnalyzerBlock(fromDirIn: string, toDir: string, folderName: string) {
		const fromDir = path.join(fromDirIn, folderName, visualText.ANALYZER_SEQUENCE_FOLDER);
		...
		// Copy over KB files
		const fromUri = visualText.analyzer.constructDir(vscode.Uri.file(path.join(fromDirIn, folderName)), anaSubDir.KB);
		const toUri = visualText.analyzer.constructDir(vscode.Uri.file(toDir), anaSubDir.KB);
		const kbFiles = dirfuncs.getFiles(fromUri, [".dict", ".kbb"]);
				...
				for (const selection of selections.reverse()) {
					if (selection.description) {
						...
						if (dirfuncs.isDir(selection.description)) {
							const files = dirfuncs.getFiles(uri, ['.nlp', '.pat']);
							for (const file of files) {
	...
	explore() {
		const dir = visualText.analyzer.getSpecDirectory();
	...
	passItemFromPath(nlpFilePath: string): PassItem {
		...
		const passItem: PassItem = seqFile.findPass('nlp', seqName);
		if (passItem.passNum) {
	...
	private openKB(seqItem: SequenceItem): void {
		if (this.notMissing(seqItem)) {
			...
			if (fs.existsSync(visualText.analyzer.getOutputDirectory().fsPath)) {
				const kbfile = this.treeFile.anaFile(seqItem.passNum, nlpFileType.KBB);
	...
	private updateTitle(): void {
		const analyzerName = visualText.getCurrentAnalyzerName();
