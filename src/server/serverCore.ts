// NLP++ language server: the handlers, for whichever process runs them.
//
// This owns every standard language feature: outline, hover, definition,
...
//
// It imports NOTHING from 'vscode', from Node or from the DOM. All analysis comes
// from the pure modules in src/language and src/format -- this file is the LSP
// adapter over them -- and it takes vscode-languageserver's common API rather
// than its /node or /browser entry. Two entries run it:
//
//   server.ts         a Node process (the extension over node-ipc, or any LSP
//                     editor with --stdio), reading the workspace from disk
//   browserServer.ts  a Web Worker, reading the files the page sends
//
// tsconfig.browser.json typechecks the browser entry with no Node types, which
// is what keeps fs, path and process out of this file and everything it imports.
//
...
import {
	Connection, TextDocuments, InitializeParams,
...
export interface StartedServer {
	// The cross-pass index, for an entry that feeds it files (memoryFiles.ts).
	index: NlpWorkspaceIndex;
	// Re-lint every open document, after the index has learned new declarations.
	refreshDiagnostics(): void;
	// Start reading messages. The entry calls it once its own handlers are on.
	listen(): void;
}

// Register every handler on `connection`, reading the workspace through `files`.
// dist/server.js passes a Node connection and the disk; dist/browserServer.js a
// worker's connection and the files the page sent. Nothing below knows which.
export function startServer(connection: Connection, files: WorkspaceFiles): StartedServer {
	const documents = new TextDocuments(TextDocument);
	const workspaceIndex = new NlpWorkspaceIndex(files);
	...
	function gatherSymbolSets(): SymbolSets {
		...
		for (const s of workspaceIndex.search("")) {
	...
	connection.onInitialized(() => {
		...
		void workspaceIndex.ensureBuilt();
	});
	...
	connection.onWorkspaceSymbol(async (params): Promise<SymbolInformation[]> => {
		await workspaceIndex.ensureBuilt();
		return workspaceIndex.search(params.query).map((s) => ({
			name: s.name,
			kind: wsKind(s),
	...
	connection.onReferences(async (params): Promise<Location[]> => {
		...
		const locations: Location[] = workspaceIndex.references(hit.word)
			.map((r) => ({ uri: r.uri, range: r.range }));
		if (params.context.includeDeclaration) {
			for (const d of workspaceIndex.lookup(hit.word)) {
	...
	connection.onPrepareRename(async (params) => {
		...
		await workspaceIndex.ensureBuilt();
		workspaceIndex.indexText(doc.uri, doc.getText()); // cover unsaved edits
		if (workspaceIndex.lookup(hit.word).length === 0) {
	...
	connection.onRenameRequest(async (params): Promise<WorkspaceEdit | undefined> => {
		...
		if (!hit) return undefined;
		await workspaceIndex.ensureBuilt();
		workspaceIndex.indexText(doc.uri, doc.getText());
		...
		for (const ref of workspaceIndex.references(hit.word)) add(ref.uri, ref.range);
		for (const d of workspaceIndex.lookup(hit.word)) add(d.uri, d.range);
	...
	connection.onSignatureHelp(async (params): Promise<SignatureHelp | undefined> => {
		...
		const decl = workspaceIndex.lookup(call.name)
	...
	documents.onDidOpen((e) => {
		workspaceIndex.indexText(e.document.uri, e.document.getText());
	...
	documents.onDidClose((e) => {
		...
		if (uri.startsWith("untitled:")) workspaceIndex.removeFile(uri);
		else void workspaceIndex.indexUri(uri);
	});
	...
	connection.onDidChangeWatchedFiles((params) => {
		for (const change of params.changes) {
			...
			if (change.type === 3) workspaceIndex.removeFile(change.uri);
			else if (!/(^|\/)(node_modules|output|[^/]*_log)\//.test(change.uri)) {
				void workspaceIndex.indexUri(change.uri);
			}
		}
	});
