...
export class NLPFile extends TextFile {
	...
	formatRule(ruleStr: string, type: reformatType = reformatType.NORMAL): string {
		// Delegates to the pure formatter engine (src/format/rulesRegion.ts) so the
		// manual reformat commands and the whole-document formatter share one
		// implementation. (#1065, #1077 fixes live there now.)
		return formatRuleText(ruleStr, type, this.getSeparator());
	}
